...
case class File(toJava: java.io.File) {
  ...
  def children: List[File] = {
    if (toJava.isFile) Nil else {
      val ls = toJava.list
      if (ls == null) throw new Error("directory does not exist or is not accessible: " + toString)
...
class StandardPrintWriter(f: File) extends
      OutputStreamWriter(new BufferedOutputStream(new FileOutputStream(f.toJava)), java.nio.charset.Charset.forName("UTF-8")) {
...
object File {
  ...
  def Reader(f: File): BufferedReader = {
    val in = new FileInputStream(f.toJava)
